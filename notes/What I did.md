...
# 2025-12-01
## 今日やったこと
- LeetCode2141: “二分探索”「最大の OK」を探すパターン
## 気付き
```
🎯 パターン一覧（まずこれだけ覚えればOK）
パターン	欲しい答え	mid の取り方	OK のときの動き	NG のときの動き
① 最小の OK を探す	最初に True になる位置	下寄り (l+r)//2	r = mid	l = mid + 1
② 最大の OK を探す	最後の True の位置	上寄り (l+r+1)//2	l = mid	r = mid - 1
③ NG/OK 法（AtCoder版）	最初 or 最後の OK	mid は (ok+ng)//2	ok を動かす	ng を動かす
```
